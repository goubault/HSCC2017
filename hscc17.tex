...
\begin{eqnarray}%{lcl}
f^{[1]}(\intvl{x_0}) & = & \left(\begin{array}{c}
...
As an example, the first component of $f^{[1]}(\intvl{x_0})$ computed in Equation (\ref{firstcompf1})
gives the interval $\left[-2.6077,-2.0766\right]$.
...
$k=2$ and by instantiating the Taylor coefficient in degree zero in $t$ with
...
(\ref{secondcompf1})~: 
\begin{equation}
\intvl{\z_1}(h,t_0,[{\z}_0]) = \left[2,2.5\right]+\left[-2.6077,-2.0766\right]t+\left[    3.5371,   13.7617\right]
\frac{t^2}{2}
\end{equation}
hence the result of Equation (\ref{outerapprox}) for $t=h=0.2$. 
...
time $t=h$, using Equation (\ref{}) for the outer-approximation of the center at time $h$ and
Equation (\ref{outerapproxJ}) for the outer-approximation of the Jacobian at time $h$~: 
\begin{multline}
%]\z[(t,t_j) = [\tilde{\z}](t,t_j,[\tilde{\z}_j]) + [\J](t,t_j,[{\z}_j])*([\overline{z_0},\underline{z_0}]-\tilde{z_0}) 
]\z[(h,0) = \left[1.9655, 1.9718\right]+\left[0.92545, 0.96808\right]\left[2.5,2\right]\\
+\left[0.20597, 0.32253\right]\left[0.15,0.1\right]
\end{multline}
Finally, using Kaucher arithmetic (see Section \ref{Kaucherar2}), we find $]\z[(h,0)=...$. 
