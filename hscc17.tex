...
the image of an input set by a non-linear vector-valued function. 
...
(http://aaflib.sourceforge.net) for affine arithmetic.
...
We use Taylor models of order 4 in time, and represent  in Figure~\ref{fig:brusselator_order4_x2_t10}, the inner and outer approximated flowpipes for variable $x_2$, up to a maximum time $t=10$. The wrapping effect is controlled by using affine arithmetic for the
coefficients of the Taylor models. Matrix preconditioning is a more classical alternative. 
However, the use of affine arithmetic is convenient for prototyping, as we rely on the {\tt aaflib} library,
but it also provides a nice trade-off between efficiency and accuracy. We see that the width of the outer-approximation 
(external lines) remains indeed stable. 
Note that the width of the inner-approximation (internal lines) can decrease until the inner-approximation becomes empty 
...
Note also that with our approach, taking order 3 Taylor models and a larger time step of 0.1, we still obtain results of very similar quality on $\gamma$, in 0.25 seconds. 
Further decreasing the precision starts degrading the quality of results. 



On Figure~\ref{ex:width_sriram_bruss1}, we represent $\gamma_{\min}$. We see that at $t=3$, the relative width of the inner-approximation over the
...
\end{figure}
On Figure~\ref{ex:width_sriram_bruss2}, we represent the evolution with time of the widths of the inner- and outer-approximations of both x1 and x2. We see that when the width of the inner-approximation on $x_2$ gets very close 
...
\end{center}
\caption{Evolution with time of the width of inner- and outer-approximations \label{ex:width_sriram_bruss2} }
\end{figure}
...
$\gamma(t=0.2) \approx 0.65$, which is this time a much better accuracy than the $\gamma(t=0.2) = 0.25$ obtained in 632 seconds in  \cite{Underapproxflowpipes}. 
...
with a prototype implementation, and showed that the method scales very well to larger-order systems of ODEs. 
