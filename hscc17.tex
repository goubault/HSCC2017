...
\title{Forward inner-approximated reachability  of continuous systems}
...
\maketitle
\begin{abstract}
In this paper, we propose a new method for inner-approximations of reachables sets of dynamical systems defined
by (possibly non-linear, and uncertain) 
ordinary differential equations. This is a notoriously difficult problem, much more intricate than 
outer-approximations, for which there exist 
well known solutions, mostly based on Taylor models. Our solution is also
based on Taylor models, but contrarily to the few methods developped recently 
for inner-approximations of non-linear ODEs, our method is a forward method,
which only needs rather inexpensive set-based methods, instead of the clever, but still complicated, backward
methods for which extra ingredients, either coming from optimisation, or involving topological criteria to be
tested (connectedness in particular), are required. 
Still, we are able to show with our prototype Matlab implementation that our method is both efficient and
precise on numerous classical examples. 
...
Our method, being based on Taylor models and being a forward method, also provides outer-approximations, naturally. The combination of both is at the heart of refined methods for verification and falsification of properties
...
the continuous part of hybrid systems, since the discrete part has been treated elsewhere \cite{HSCC2014}. 

\paragraph{Contributions:}

This paper extends the work presented in ~\cite{sas07,hscc14}, where the authors proposed an approach for direct forward inner-approximated reachability of 
...
computing inner-approximations of the flow of uncertain initial value problems. 
There are two main ingredients for our method. The first one is the fact that one only need outer-approximations
in fact, for deriving inner-approximations, not only of the reachable states of the dynamics but also
for the ``variational equations'', including the dynamics of the Jacobian of the solutions with respect to the
initial values. This is made possible by using a generalized mean value theorem, that we introduce in the
preliminaries, Section \ref{prelim}. 

\ForAuthors{A continuer}
...
%- guards \\
- experiments with a matlab implementation and tentative comparisons to existing work (but dual approaches - mostly bacward inner reachability ? - combination to be studied ?) 
...
even non-polynomial, of the form : 
\begin{equation}
\dot{x}(t) = f(x,p,t)
\label{eq:flow}
\end{equation}
...
\SP{Def de forward inner approximation}
...
% RAJOUT EG
Outer and inner-approximations for linear systems has been well studied, \cite{LeGuernic09,GirardLinear06}.
% FIN RAJOUT EG

There exist a few methods to compute global inner-approximations of the image of non-linear vector-valued 
functions, mostly based on bisections of the input domain, see for instance \cite{Goldsztejn2010}, later
extended by the authors in \cite{rc13}, or inner approximating sets of (semi-algebraic)
constraints \cite{HenrionLouembet}. But these bisections are very costly if an accurate approximation is needed, 
...
% RAJOUT EG
Under-approximate flowpipes \cite{Underapproxflowpipes}.
% FIN RAJOUT EG
Some recent work~\cite{underapprox16} propose a computation of backward inner-approximation. The problem is in some sense 
...
ranges over boxes. The set of classical intervals is 
denoted by $\I = \{ [a,b], \; a\in \bbr, b \in \bbr, a \leqslant b\}$. In what follows, 
uncertain quantities defined in intervals are noted in bold, outer-approximating interval enclosures are noted in bold plus brackets, 
and inner-approximating intervals are noted in bold plus outward brackets.
 An outer approximating extension of a function $f: \R^n \rightarrow \R $ is a function 
$[\f]: \I^n\rightarrow \I$ such that $\forall \x \in \I^n, \mbox{range}(f,\x)=\{f(x), x \in \x\} \subseteq [\f](\x)$.
...
bounds are not ordered, and the Kaucher arithmetic~\cite{Kaucher} on these intervals.  The set of generalized
...
We now decompose $\K$ in ${\cal P} = \{\x=[\underline x,\overline x], \; \underline x \geqslant 0 \wedge
...
Kaucher multiplication $\x \times \y$ is described in Table \ref{tabmult}. 
...
Kaucher arithmetic defines a generalized interval natural extension (see~\cite{gold1})~:
\begin{proposition}
\label{prop1}
...
\begin{equation}
\tilde{f}(\x) = f(\tilde x) + \sum_{i=1}^{n} [\bfm{\Delta}_i] (\x_i - \tilde{x}_i).
\label{Taylor}
\end{equation}
...
Computing an enclosure of the solution of an initial value problem is the object of Section~\ref{sec:Taylor}.  

\subsection{Enclosing the flow of an uncertain ODE with interval Taylor methods~\label{sec:Taylor}}
...
at time $t$ for initial conditions in ${\cal Z_0}$ at $t_0$. We define  time grid $t_0 < t_1 < \ldots < t_N$, and assume
...
[\z](t,t_j,[{\z}_j]) = [{\z}_j] + \sum_{i=1}^{k-1} (t-t_j)^i f_q^{[i]}([{\z}_j])+ (t-t_j)^k f_q^{[k]}([{\r}_{j+1}]),
\label{eq:TM}
\end{equation}
...
We thus need an (outer-approximating) enclosure of $z(t,\tilde{z_0})$ for some $\tilde{z_0} \in \z_0$, and of its Jacobian with respect to $z_0$, evaluated over range $\z_0$, 
...
= \sum_{k=1}^n \frac{\partial f_i}{\partial z_k}(z) . J_{kj}(t,z_0)
\label{eq:IVP_jac}
\end{equation} 
...
The corresponding variational equations, for the Jacobian of the solutions $x_i$ with respect to the initial
...
\label{jacobian}
\end{eqnarray}
\noindent with initial conditions (at time 0) $J_{i,j}=\delta_{i,j}$.
\end{example}
...
\begin{equation}
[\z](t,t_j,[{\z}_j]) = [{\z}_j] + \sum_{i=1}^{k-1} (t-t_j)^i f^{[i]}([{\z}_j])+ (t-t_j)^k f^{[k]}([{\r}_{j+1}]).
\label{eq:TM1}
\end{equation}
\begin{equation}
[\tilde{\z}](t,t_j,[\tilde{\z}_j]) = [\tilde{\z}_j] + \sum_{i=1}^{k-1} (t-t_j)^i f^{[i]}([\tilde{\z}_j])+ (t-t_j)^k f^{[k]}([\tilde{\r}_{j+1}]).
\label{eq:TM2}
\end{equation}
\begin{eqnarray}
[\J](t,t_j,[{\z}_j])& =& [{\J}_j] + \sum_{i=1}^{k-1} (t-t_j)^i \mbox{Jac}_x(f^{[i]})([{\r}_{j+1}]) [{\J}_j] \\&& +  (t-t_j)^k \mbox{Jac}_x(f^{[k]})([{\r}_{j+1}]) [\bfm{R}_{j+1}]
\label{eq:TM3}
\end{eqnarray}
...
\ForAuthors{J'essaie de coller \`a \ref{eq:TM1} mais le
fait que l'indice en pas de temps soit en bas g\^ene ; on ne pourrait pas le mettre en haut
plut\^ot? ;-) On a d\'ej\`a les indices en bas naturellement, pour les \'etats et la jacobienne.}

\begin{eqnarray}
\int{x_i}(t) & = & \int{(x_i)_0} + {f_i}^{[0]}(\int{x_0}) t + f_i^{[1]}(\int{r_1}) \frac{t^2}{2} \\
\int{J_{i,j}}(t) & = & \int{(J_{i,j})_0}+Jac_x(f_{i}^{[0]})(\int{r_1})\int{R_1}^j t + Jac_x(f_{i}^{[1]})(\int{r_1})\int{R_1}^j \frac{t^2}{2} \\
\label{TaylorJ}
\end{eqnarray}
...
%& & \ \ \ -\frac{9}{2}x_1^3x_2^2+\frac{9}{4}x_1^4x_2 \\
Jac_x(f^{[1]})_{1,1}(x_1,x_2)(J_{1,1},J_{2,1}) & = & (3x_2f_1+3x_1f_2)J_{1,1}+(-2+3x_1x_2)Jac_x(f^{[0]}_{1,1}(x_1,x_2)(J_{1,1},J_{2,1}) \\ 
& & +3x_1f_1J_{2,1}+\frac{3}{2}x_1^2{Jac_x(f^{[0]})}_{2,1}(x_1,x_2)(J_{1,1},J_{2,1}) 
...
\end{eqnarray} %$$
\noindent where $Jac_x(f^{[1]})_{1,1}(x_1,x_2)(J_{1,1},J_{2,1})$ and ${Jac_x(f^{[0]})}_{2,1}(x_1,x_2)(J_{1,1},J_{2,1})$ are given in Equations \ref{jacobian} (they are the right hand side
...
\end{eqnarray}%$$
\ForAuthors{Ptet remettre les valeurs flottantes?}
\end{example}
