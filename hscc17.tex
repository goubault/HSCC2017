...
\begin{equation} \dot z(t) = f_q(z) ,
\label{eq:flowb}
\end{equation}
\begin{equation}
 \gamma_e \left( z(t) \right) = 0. 
\label{eq:guardb}
 \end{equation}
...
\end{equation}
\begin{equation}
[\tilde{\z}](t,t_j,[\tilde{\z}_j]) = [\tilde{\z}_j] + \sum_{i=1}^{k-1} (t-t_j)^i f_q^{[i]}([\tilde{\z}_j])+ (t-t_j)^k f_q^{[k]}([\tilde{\r}_{j+1}]).
\label{eq:TM2}
\end{equation}
\begin{eqnarray}
[\J](t,t_j,[{\z}_j])& =& [{\J}_j] + \sum_{i=1}^{k-1} (t-t_j)^i \mbox{Jac}_x(f_q^{[i]})([{\r}_{j+1}]) [{\J}_j] \\&& +  (t-t_j)^k \mbox{Jac}_x(f_q^{[k]})([{\r}_{j+1}]) [\bfm{R}_{j+1}]
\label{eq:TM3}
\end{eqnarray}
