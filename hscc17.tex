...
\subsection{Step 1: computating the rough enclosures}
% HOP
\label{roughenc}
...
\end{multline} %$$
\noindent where $Jac_x(f^{[1]})_{11}$ %(x_1,x_2)(J_{1,1},J_{2,1})$ 
and ${Jac_x(f^{[0]})}_{21}$ %(x_1,x_2)(J_{1,1},J_{2,1})$ 
...
In particular, as we rely on Taylor models, if necessary we can improve the quality by using higher-order models. 
And as we know that the exact enclosure of the uncertain system lies between the inner and outer-approximated flows, 
we can bound the approximation error at each instant, and use this information to refine the appproximation when this error becomes larger than some threshold. 
%\end{remark}
...
In the following experiments, we now use Taylor models of order 4, and represent respectively in Figures~\ref{fig:brusselator_order4_x1_t10} and~\ref{fig:brusselator_order4_x2_t10}, the inner and outer approximated flowpipes for variables $x1$ and $x2$, with an evaluation in affine arithmetic, up to a maximum time $t=10$.   
\ForAuthors{commenter que la sous-approx devient vide puis re-apparait} 
%(cf \ref~{practicalissues})
...
\SP{reflechir a ce qu'on peut en deduire sur le systeme (cycle?), et si on veut que je fasse d'autres experiences ?}
...
\end{center}
\begin{center}
\epsfig{file=sriram_bruss_order4_t4_widthx1.png,clip=,width=8cm}
\end{center}
\begin{center}
\epsfig{file=sriram_bruss_order4_t4_widthx2.png,clip=,width=8cm}
\end{center}
\begin{figure}
\begin{center}
...
Ce que je veux faire aussi lundi c'est d'essayer avec un pas plus grand, je ne crois pas qu'il y a besoin d'un si petit pas, enfin je ferai ca apres, d'abord les urgences...

Oui bien sur pour les urgences ;-) Mais oui un pas plus grand peut 
parfaitement marcher. Dans tous les cas, comparer le temps sur et sous
approx, ca permet de donner un facteur pour le temps, du essentiellement
a l'implem.}

\SP{Add elements allowing to compare the cost of inner-approx comapred to inner-approx of same degree: where to put this and on which example, see later}  
...
\begin{center}
\epsfig{file=cav16_bruss_order4_x1.png,clip=,width=8cm}
\end{center}
\begin{center}
\epsfig{file=cav16_bruss_order4_x2.png,clip=,width=8cm}
\end{center}
\begin{center}
