...
\maketitle
\begin{abstract}
In this paper, we propose a new method for inner-approxima\-tions of reachable sets of dynamical systems defined
...
well known solutions, mostly based on Taylor models. Our solution for inner-approximations is also
based on Taylor models, but contrarily to the few methods developped recently, our method is a forward method,
which only needs rather inexpensive set-based methods, instead of the clever, but still complicated, backward
methods for which extra ingredients, either coming from optimisation, or involving topological criteria, are required. 
...
Our method, being based on Taylor models and being a forward method, also provides outer-approximations, naturally. The combination of both inner and outer approximations 
can be seen as the basis for verification and falsification of properties
...
[\z](t,t_j,[{\z}_j]) =  [{\z}_j] + \sum_{i=1}^{k-1} \frac{(t-t_j)^i}{i !} f^{[i]}([{\z}_j])\\
 + \frac{(t-t_j)^k}{k !} f^{[k]}([{\r}_{j+1}]),
\label{eq:TM}
\end{multline}
where the Taylor coefficients $f^{[i]}$, which are the $i$th Lie derivative of $f$ along
vector field $f$, are defined inductively, and 
can be computed by automatic differentiation as follows (for all $k=1,\ldots,n$)~: 
\begin{eqnarray}
f^{[0]}_k & = & f_k \\
f^{[i+1]}_k & = & \sum_{j=1}^n \frac{\partial{f^{[i]}_k}}{\partial z_j} f_j
\label{Lie1}
\end{eqnarray}
...
$H^{[i]}$ the $i$th Lie derivative of $H$ along the (augmented) vector field $H$. More
...
\begin{equation}
H^{[1]} = \sum_{i=1}^n \frac{\partial H}{\partial z_i} f_i \\
+\sum_{k,l=1}^n \frac{\partial H}{\partial J_{jk}} \left(
...
$$\begin{array}{rcl}
\dot{\tilde{H}}(t) & = & \sum_{i=1}^{n} \frac{\partial H}{\partial z_i}(t)
\dot{z}_i(t)+\sum_{k,l=1}^n \frac{\partial H}{\partial J_{kl}}(t) \dot{J}_{kl}(t)
\end{array}$$
...
\end{multline}
\begin{multline}
[\tilde{\z}](t,t_j,[\tilde{\z}_j]) = [\tilde{\z}_j] + \sum_{i=1}^{k-1} \frac{(t-t_j)^i}{i !} f^{[i]}([\tilde{\z}_j])\\+ \frac{(t-t_j)^k}{k !} f^{[k]}([\tilde{\r}_{j+1}]).
\label{eq:TM2}
\end{multline}
\begin{multline}
[\J](t,t_j,[{\z}_j]) = [{\J}_j] + \sum_{i=1}^{k-1} \frac{(t-t_j)^i}{i !} \mbox{Jac}_x(f^{[i]})([{\r}_{j+1}]) [{\J}_j] \\ +  \frac{(t-t_j)^k}{k !} \mbox{Jac}_x(f^{[k]})([{\r}_{j+1}]) [\bfm{R}_{j+1}]
\label{eq:TM3}
\end{multline}
...
The Equation (\ref{eq:TM3}) is equivalent since the two derivative (the Jacobian calculation
and the Lie derivative) commute. Equation (\ref{eq:TM3}) 
...
%& & \ \ \ -\frac{9}{2}x_1^3x_2^2+\frac{9}{4}x_1^4x_2 \\
\begin{multline}
Jac_x(f^{[1]})_{1,1}(x_1,x_2)(J_{1,1},J_{2,1}) = (3x_2f_1+3x_1f_2)J_{1,1}\\
+(-2+3x_1x_2)Jac_x(f^{[0]}_{1,1}(x_1,x_2)(J_{1,1},J_{2,1}) \\ 
