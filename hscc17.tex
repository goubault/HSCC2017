...
We demonstrate with a Matlab implementation that our method is both efficient and
...
variables (the $z_i$ and $J_{ij}$). We call the corresponding vector field $F$ and
...
For $i=1$, we have, by definition $\mbox{Jac}_z(f^{[1]})
=\mbox{Jac}_z(f)=(\mbox{Jac}_z(f))^{[1]}$. Suppose now we have, as an induction
step $\mbox{Jac}_z(f^{[i]})=(\mbox{Jac}_z(f))^{[i]}$. We now write~:
$$
%\begin{array}{rclcl}
\mbox{Jac}_z(f^{[i+1]})_{pq} 
%& 
= 
%& 
...
\noindent hence, 
\begin{equation}
\mbox{Jac}_z(f^{[i+1]})_{pq} =  \sum\limits_{k,l=1}^n \left(\frac{\partial^2 f_p^{[i]}}{\partial z_k \partial z_l} f_l
...
$$\begin{array}{rcl}
(\mbox{Jac}_z(f)_{pq})^{[i+1]} & = &
(\mbox{Jac}_z(f^{[i]})_{pq})^{[2]}
\end{array}$$
...
$$\begin{array}{rcl}
(\mbox{Jac}_z(f^{[i]}))_{pq} = \sum\limits_{k=1}^n \frac{\partial f^{[i]}_p}{\partial z_k} J_{kq}
\end{array}$$
\noindent we have~:  
\begin{multline}
(\mbox{Jac}_z(f)_{pq})^{[i+1]}  = 
...
\end{multline}
\noindent which is thus seen to be equal to $\mbox{Jac}_z(f^{[i+1]})_{pq}$ by
...
Equation (\ref{eq:TM2}) is required to estimate the inner-approximations as in Equation (\ref{eq:inner_cont}) : we need to propagate a ``center'' in 
...
It must be noted that the quality of the inner-approxima\-tion is strongly linked to the quality of the outer-approximation. 
...
%******************************************************************************
%
We presented an approach to compute inner-approximating flowpipes of uncertain ODEs, that extends in a simple way Taylor-based 
