...
\label{eq:TM}
\end{multline}
where the Taylor coefficients $f^{[i]}$ can be computed by automatic differentiation. \\
...
but the equivalent Equation (\ref{eq:TM3}) is simpler to compute since we already computed
the Lie derivative of $f$, in Equation (\ref{eq:TM2}). The Jacobian calculation is afterwards
inexpensive. 
\end{remark}
