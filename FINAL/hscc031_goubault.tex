...
\doi{http://dx.doi.org/10.1145/3049797.3049811}
...
%
% 1st. author
\alignauthor Eric Goubault \\ %\titlenote{Dr.~Trovato insisted his name be first.}\\
       \affaddr{LIX, Ecole Polytechnique, CNRS, }\\
       \affaddr{Universit\'e Paris-Saclay, 91128 Palaiseau, France} \\
       \affaddr{\small goubault@lix.polytechnique.fr}
% 2nd. author
\alignauthor Sylvie Putot \\ %\titlenote{The secretary disavows any knowledge of this author's actions.}\\
       \affaddr{LIX, Ecole Polytechnique, CNRS, } \\
       \affaddr{Universit\'e Paris-Saclay, 91128 Palaiseau, France}\\
       \affaddr{\small putot@lix.polytechnique.fr}
}
...
\begin{tikzpicture}[remember picture,overlay]
  \node[anchor=north east,inner sep=0pt] at ($(current page.south west)+(8.1in,10.6in)$) {
  \includegraphics[scale=0.3]{hscc-badge2}
  };
  \end{tikzpicture}
...
\keywords{Inner-approximation; Taylor models; affine arithmetic; modal intervals; reachability} 
...
%CHECK \cite{kaynama2012computing,DBLP:phd/hal/LeGuernic09}
%\cite{Cardaliaguet2007} bof, vois pas trop le rapport
...
\KwResult{$[\z](t,\z_0)$ and $]\z[(t,\z_0)$ over $t=[t_0,t_N]$}
\KwInit{ $j=0$, $t_j=t_0$, $[\z_j]=\z_0$, $[\tilde{z}_j]=\tilde{z}_0$, $[\J_j] = Id$}
\While{$j<N-1$}{
...
\end{multline}
\begin{multline}
\hspace*{-0.5cm} [\J](t,t_j,[{\z}_j]) = [{\J}_j] + \sum\limits_{i=1}^{k-1} \frac{(t-t_j)^i}{i !} \mbox{Jac}_zf^{[i]}([{\z}_{j}]). [{\J}_j] \\ +  \frac{(t-t_j)^k}{k !} \mbox{Jac}_zf^{[k]}([{\r}_{j+1}]). [\bfm{R}_{j+1}]
\label{eq:TM3}
\end{multline}
...
\underline{Step 4}. %initialize the next iteration:
 $[{\z}_{j+1}]=[\z](t_{j+1},t_j,[{\z}_j])$, $[\tilde{\z}_{j+1}]=[\tilde{\z}](t_{j+1},t_j,$ $[\tilde{\z}_j])$, $[\J_{j+1}] = [\J](t,t_j,[{\z}_j]) $
}
% }%         %    --------
%   }%                                %
%  \end{center}%                      %
...
as in our prototype. % and will compare it with interval computations, that we
...
%\EG{Tu n'es pas tres positive sur affine arithmetic versus preconditioning? 
%Moi j'avais l'impression quand meme que ca pouvait etre mieux aussi
%bien precision que efficiency avec affine arithmetic? J'ai un peu reedite 
%les phrases au dessus - et utilise l'environnement url}
% Sylvie: j en'ai pas ete tre assertive car rien de prouve; mais ok j'ai fait plus positif ;)
...
%\EG{La qualite du png est moyenne? Ptet sous apercu quand tu change de tiff
%en png, il faut mettre qualite max ou qqchose comme ca?}
%Sylvie: je te laisse voir si tu arrives a qqchose (?)
\end{figure}
...
%\EG{Je n'arrive pas a trouver une formulation qui me paraisse vraiment anglaise,
%peut-etre celle-ci, mais pas de verbe et pour moi c'etait the larger, the better
%ou si tu precises, il faut un verbe pour faire une phrase? sinon 
%je propose the larger this ratio is, the better the approximation gets ?}
% Moui, je ne sais pas...
...
methods for outer-approximation. %We assessed the good behaviour of the inner compared to the classical outer-approximations, 
...
\newblock {\em Reliable Computing}, 18:117--143, 2013.
...
\bibitem{DBLP:conf/nolcos/KordaHJ13}
...
\newblock {\em {ACM} Trans. Math. Soft.}, 2006.
...
\newblock Validated solutions of initial value problems for ordinary
  differential equations.
\newblock {\em Appl. Math. Comp.}, 1999.
...
\newblock {NLTOOLBOX:} {A} library for reachability computation of nonlinear
  dynamical systems.
\newblock In {\em ATVA}, pages 469--473, 2013.

\bibitem{underapprox16}
B.~Xue, Z.~She, and A.~Easwaran.
\newblock Under-approximating backward reachable sets by polytopes.
\newblock In {\em CAV}, 2016.

\bibitem{Zgliczynski2002}
P.~Zgliczynski.
\newblock {$C^1$} {L}ohner algorithm.
\newblock {\em Foundations of Computational Mathematics}, 2(4):429--465, 2002.

\bibitem{falsification2}
